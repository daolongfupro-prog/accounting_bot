# В файле admin_edu.py замени колбэк для списка списаний
@router.callback_query(F.data == "edu_deduct_list") # Соответствует новой кнопке
async def edu_deduct_list(callback: CallbackQuery):
    ...
    await callback.message.edit_text("👇 Выберите ученика для списания:", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))

# А это для кнопки Статистика
...
async def show_edu_stats(callback: CallbackQuery):
    ...
    await callback.message.edit_text(text, parse_mode="HTML", reply_markup=get_edu_admin_kb())
